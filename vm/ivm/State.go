...
// Copy returns a duplicate state.
func (s *State) Copy() State {
	...
	copy(newState.Registers[:], s.Registers[:])
	return newState
}
...
const fprintRegistersColumnSize = 4

// FprintRegisters prints all the registers to the given writer.
func (s State) FprintRegisters(w io.Writer) error {
	for i, regVal := range s.Registers {
		var err error
		if i%fprintRegistersColumnSize > 0 {
			_, err = fmt.Fprint(w, "  ")
			if err != nil {
				return err
			}
